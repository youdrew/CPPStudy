...
int Martix3Build(int x,int y,int z);
...
int main(){
    ...
    //—————————————【方法2：将三维数组的第一个二维数组赋值给*ptr7，将指针赋值给二维数组，所以每次增量都是二纬的】————
    int (*ptr7)[2][3]  = Var3;//和int (*ptr6)[4][2][3]=&Var3;赋值方式差别不大，都是首地址，只是每次+1的步长不一样
    cout << "*ptr7:   " << *ptr7 << endl;
    cout << "*ptr7+3: " <<*ptr7+3 << endl; //第一个纬度里+1，每次加一个一维数组

    //—————————————【方法3：将指针赋值给一维数组，所以每次增量都是一纬的】————————————————
    int (*ptr8)[3]  = Var3[0]; //或者a[1]、a[2]
    cout << "*ptr8:   " << *ptr8 << endl;
    cout << "*ptr8+1: " << *ptr8+1 << endl;

    //—————————————【方法4：直接取到具体到值，取到到是各纬度的第一个】———————————————————
